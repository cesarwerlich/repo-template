...
This template does not ship Gemini slash-command files. Use Gemini's native skills discovery, or explicitly reference a skill by path:

```markdown
Use .agents/skills/planning-and-task-breakdown/SKILL.md to break this work into tasks.
```

If you later add `.gemini/commands/`, keep them as thin wrappers around the same skills so the workflow stays consistent across tools.
